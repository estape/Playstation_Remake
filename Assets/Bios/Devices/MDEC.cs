...
namespace ProjectPSX.Devices
{
    public class MDEC
    {
        ...
        {//64 unsigned parameter bytes for the Luminance Quant Table (used for Y1..Y4)
            for (int i = 0; i < 32; i++)
            { //16 words for each table
            ...
            for (int i = 0; i < 32; i++)
            { //16 words continuation from buffer
                ushort value = inBuffer.Dequeue();
        ...
        {//64 signed halfwords with 14bit fractional part
            for (int i = 0; i < 64; i++)
            { //writed as 32 words on buffer
